#' Negative Binomial process modeling of sequencing count data
#'
#' This function fits NBP model to counts in different conditions and returns the
...
#' @return posterior samples of normalized rate parameters of Negative Binomial
#' @examples
...
#' @export
NBP <- function(y, groups, randtry=1, Burnin=500L, Collections=1000L)
{
    set.seed(randtry)
    ...
    for (iter in 1:iterMax)
    {
        ...
        for (g in glevels)
        {
            ...
            Kj <- length(idx.nz[[g]])
            idx <- idx.nz[[g]]
            gamma0[g] <- rgamma(1, e0 + Kj, rate = f0 - log(max(c[g]/(c[g]+J), realmin)))
            r[idx, g] <- rgamma(Kj, rowSums(y[idx,subg]), rate = c[g]+J)
            r_star[g] <- rgamma(1, gamma0[g], rate = c[g]+J)
            mass <- sum(r[idx, g]) + r_star[g]
            c[g] <- rgamma(1, c0 + gamma0[g], rate = d0 + mass)

            if (iter>Burnin)
            {
                post.r[idx, (g-1)*Collections + iter-Burnin] <- r[idx, g]/sum(r[,g])
            }
        }
    }
